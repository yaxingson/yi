...
const which$1 = (cmd, opt, cb) => {
  ...
  const step = i => new Promise((resolve, reject) => {
    ...
    const pCmd = path$f.join(pathPart, cmd);
...
function resolveCommandAttempt(parsed, withoutPathExt) {
    ...
    try {
        resolved = which.sync(parsed.command, {
            ...
            pathExt: withoutPathExt ? path$e.delimiter : undefined,
        });
    } catch (e) {
        /* Empty */
    } finally {
    ...
    if (resolved) {
        resolved = path$e.resolve(hasCustomCwd ? parsed.options.cwd : '', resolved);
    }
...
function parseNonShell(parsed) {
    ...
    if (parsed.options.forceShell || needsShell) {
        ...
        parsed.command = path$d.normalize(parsed.command);
...
const npmRunPath = ({
	...
	const cwdPath = path$g.resolve(cwdString);
	const result = [];
	...
	return [...result, pathOption].join(path$g.delimiter);
};
...
const applyPreferLocal = (result, cwdPath) => {
	...
	while (previous !== cwdPath) {
		result.push(path$g.join(cwdPath, 'node_modules/.bin'));
		previous = cwdPath;
		cwdPath = path$g.resolve(cwdPath, '..');
	}
};
...
const applyExecPath = (result, execPath, cwdPath) => {
	...
	result.push(path$g.resolve(cwdPath, execPathString, '..'));
};
...
const handleArguments = (file, args, options = {}) => {
	...
	if (process$3.platform === 'win32' && path$g.basename(file, '.exe') === 'cmd') {
		// #116
		args.unshift('/q');
	}
...
-  already used by option '${matchingOption.flags}'`);
  ...
  */
  ...
  _executeSubCommand(subcommand, args) {
    ...
    function findFile(baseDir, baseName) {
      ...
      const localBin = path$c.resolve(baseDir, baseName);
      if (fs$i.existsSync(localBin)) return localBin;

      // Stop looking if candidate already has an expected extension.
      if (sourceExt.includes(path$c.extname(baseName))) return undefined;

      // Try all the extensions.
      const foundExt = sourceExt.find(ext => fs$i.existsSync(`${localBin}${ext}`));
    ...
    if (this._scriptPath) {
      ...
      try {
        resolvedScriptPath = fs$i.realpathSync(this._scriptPath);
      } catch (err) {
        resolvedScriptPath = this._scriptPath;
      }
      executableDir = path$c.resolve(path$c.dirname(resolvedScriptPath), executableDir);
    }
    ...
    if (executableDir) {
      ...
      if (!localFile && !subcommand._executableFile && this._scriptPath) {
        const legacyName = path$c.basename(this._scriptPath, path$c.extname(this._scriptPath));
    ...
    launchWithNode = sourceExt.includes(path$c.extname(executableFile));
 ...
 - ${executableDirMessage}`;
  ...
  nameFromFilename(filename) {
    this._name = path$c.basename(filename, path$c.extname(filename));
...
const ROOT_PATH = path$g.resolve(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('yi.cjs', document.baseURI).href))), '..', '..');
const CURRENT_PATH = process.cwd();

function getPkgInfo() {
    return __awaiter(this, void 0, void 0, function* () {
        const pkg = yield promises$1.readFile(path$g.resolve(ROOT_PATH, 'package.json'), 'utf-8');
...
var unpublish = new Command('unpublish');
...
function serveAction(entry) {
    return __awaiter(this, void 0, void 0, function* () {
        const defaultEntries = ['index.html', 'App.vue', 'App.tsx', 'App.svelte'];
        entry = entry || defaultEntries.find(item => lib.existsSync(path$g.resolve(CURRENT_PATH, item)));
        if (!entry)
            throw '';
        path$g.extname(entry).slice(1);
        const port = yield getPorts({ port: [9527, 8000, 9000] });
...
function genCompAction(path) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!/\.(vue|tsx|svelte)$/.test(path))
            return;
        const componentName = path$g.basename(path);
        console.log(componentName);
    });
}
function genTestAction(path) {
    return __awaiter(this, void 0, void 0, function* () {
    });
}
...
var analysis = new Command('analysis');
...
function setCommand() {
    return __awaiter(this, void 0, void 0, function* () {
        const { version, scripts } = yield getPkgInfo();
